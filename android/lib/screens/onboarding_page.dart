...
class _OnboardingPageState extends State<OnboardingPage> {
  ...
  Future<void> _deployAndPair() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => deploymentStatus = 'New server instance is Live. Verifying backend 0.4.10…');
      final healthy = await deployment.waitUntilHealthy(result.serverUrl);
      if (!healthy) throw Exception('Render did not expose backend 0.4.10 after deployment. Confirm the repository contains the current backend folder and inspect the latest Render deploy logs.');
      if (!mounted) return;
