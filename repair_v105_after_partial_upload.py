...
VERIFIED_BASE_COMMIT = "af339ac48e600b38bebababbb048e77464be3900"
REPAIR_BASE_COMMIT = "3bf8794c21ac30f61e1ccf84e452fc65785187a4"
ROOT = Path(__file__).resolve().parent


def fail(message: str) -> None:
    raise SystemExit(f"v1.0.5 repair aborted: {message}")
...
def ensure_baseline() -> None:
    head = git_head()
    if head != REPAIR_BASE_COMMIT:
        fail(f"expected failed partial-upload commit {REPAIR_BASE_COMMIT}, but HEAD is {head}. Do not stack this repair on a different commit.")
    for args, label in [
    ...
    # This repair is only for the exact partial-upload failure: the v1.0.5
    # additive files must already be committed, while existing release wiring is still v1.0.4.
    for required in [
        ...
        if not (ROOT / required).is_file():
            fail(f"partial-upload repair prerequisite missing: {required}")
...
def patch_readme_manifest_state() -> None:
    ...
    manifest = {
        ...
        "version": "1.0.5",
        "maintenance_baseline_commit": VERIFIED_BASE_COMMIT,
    ...
    state = {
        ...
        "branch": "main",
        "verified_maintenance_commit": VERIFIED_BASE_COMMIT,
...
def exclude_delta_helpers() -> None:
    ...
    entries = ["/apply_v105_human_boundary.py", "/V105_DELTA_README.md", "/repair_v105_after_partial_upload.py", "/V105_REPAIR_README.md"]
    changed = False
...
def cleanup_accidentally_tracked_helpers() -> None:
    # The failed partial upload committed the original overlay helper files. They
    # are deployment tooling, not product source. Remove them from the repaired tree.
    for name in ["apply_v105_human_boundary.py", "V105_DELTA_README.md"]:
        target = ROOT / name
        if target.exists():
            target.unlink()
...
def regenerate_manifests() -> None:
    ...
    paths = sorted(set(tracked + new_files) - {"FILE_MANIFEST.txt", "SHA256SUMS.txt", "apply_v105_human_boundary.py", "V105_DELTA_README.md", "repair_v105_after_partial_upload.py", "V105_REPAIR_README.md"})
...
def main() -> None:
    ...
    cleanup_accidentally_tracked_helpers()
    regenerate_manifests()
    print("v1.0.5 repair applied successfully to failed partial-upload commit 3bf8794.")
    print("Next: run backend tests and Flutter analysis/tests, inspect git diff, then commit and push. GitHub CI remains authoritative.")
