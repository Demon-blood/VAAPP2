...
def test_scheduler_start_uses_timezone_aware_now() -> None:
    scheduler_source = (_root() / "backend" / "app" / "services" / "scheduler.py").read_text()
    assert "datetime.now(ZoneInfo(settings.default_timezone))" in scheduler_source
