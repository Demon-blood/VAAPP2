...
def test_v100_release_identity_is_consistent() -> None:
    ...
    assert 'REQUIRED_ANDROID_VERSION = "1.0.0"' in version
    assert 'version = "1.0.0"' in _read("backend/pyproject.toml")
    assert "version: 1.0.0+42" in _read("android/pubspec.yaml")
    workflow = _read(".github/workflows/android-release.yml")
    assert "Full-Time-VA-Android-v1.0.0.apk" in workflow
    assert "Full-Time VA Android v1.0.0" in workflow


def test_android_uses_one_runtime_release_contract() -> None:
    contract = _read("android/lib/release_contract.dart")
    assert "const String appRelease = '1.0.0';" in contract
    assert "const String minimumBackendVersion = '1.0.0';" in contract
...
def test_phone_deployment_requires_v100_backend_and_has_no_legacy_floor() -> None:
    ...
    assert "backend 1.0.0 or newer" in phone_docs
...
def test_v100_release_metadata_points_to_verified_phase9_gate() -> None:
    ...
    assert '"release": "1.0.0"' in manifest
    assert '"android_version": "1.0.0+42"' in manifest
    assert '"baseline_commit": "7e8be1f82cb86c66ae07b2b90fe2173858757aa7"' in manifest
    assert '"verified_baseline_actions_run": 40' in state
    assert '"verified_baseline_actions_conclusion": "success"' in state
    assert "GitHub Actions run #40 completed successfully" in handoff
