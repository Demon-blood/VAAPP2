...
def test_v101_release_identity() -> None:
    assert 'APP_VERSION = "1.0.1"' in _read("backend/app/core/version.py")
    assert 'REQUIRED_ANDROID_VERSION = "1.0.1"' in _read("backend/app/core/version.py")
    assert 'version = "1.0.1"' in _read("backend/pyproject.toml")
    assert "version: 1.0.1+43" in _read("android/pubspec.yaml")
    assert "const String appRelease = '1.0.1';" in _read("android/lib/release_contract.dart")
    assert "const String minimumBackendVersion = '1.0.1';" in _read("android/lib/release_contract.dart")
