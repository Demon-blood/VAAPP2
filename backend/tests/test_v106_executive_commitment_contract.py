...
def test_v106_release_contract_and_routes():
    ...
    assert 'APP_VERSION = "1.0.6"' in read("backend/app/core/version.py")
    assert 'version = "1.0.6"' in read("backend/pyproject.toml")
    assert "version: 1.0.6+49" in read("android/pubspec.yaml")
    assert "appRelease = '1.0.6'" in read("android/lib/release_contract.dart")
