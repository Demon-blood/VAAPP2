...
def test_release_version_alignment() -> None:
    root = _root()
    assert 'APP_VERSION = "0.7.2"' in (root / "backend/app/core/version.py").read_text()
    assert 'version = "0.7.2"' in (root / "backend/pyproject.toml").read_text()
    assert "version: 0.7.2+30" in (root / "android/pubspec.yaml").read_text()
    workflow = (root / ".github/workflows/android-release.yml").read_text()
    assert "Full-Time-VA-Android-v0.7.2.apk" in workflow
