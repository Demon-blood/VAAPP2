...
def test_v090_release_identity_and_core_routes() -> None:
    root = _root()
    assert 'APP_VERSION = "0.9.1"' in (root / "backend/app/core/version.py").read_text()
    assert 'version = "0.9.1"' in (root / "backend/pyproject.toml").read_text()
    assert 'version: 0.9.1+34' in (root / "android/pubspec.yaml").read_text()
    workflow = (root / ".github/workflows/android-release.yml").read_text()
    assert "Full-Time-VA-Android-v0.9.1.apk" in workflow
...
def test_v090_android_has_professional_operator_view() -> None:
    ...
    assert "_versionAtLeast(backendVersion, '0.9.1')" in state
