...
- Persistent SQLAlchemy/PostgreSQL workflow runs and jobs with idempotency keys, dependency gating, retries, exponential backoff, dead-letter handling, leases, heartbeat and watchdog recovery.
- APScheduler now enqueues durable work; business execution is owned by persisted workers rather than in-memory scheduler calls.
- Unified Autopilot intents for full VA runs, Gmail, banking, contacts, connectors, housekeeping and bill lifecycle execution.
- Bill lifecycle automatically starts when a real bill is detected and retains existing creditor approval, exact IBAN, amount-limit, account-scope, balance and safety-reserve controls.
- Strong Customer Authentication remains a human-only exception: the app surfaces the real bank authorization URL instead of pretending payment completed.
- Daily VA briefing with important mail, tasks, upcoming bills, live Google Calendar appointments, support cases, orders, subscriptions, recent activity and a final `Needs you` exception list.
- Operations profile combines explicit preferences with existing learned safe sender rules, approved creditors and approved payment accounts.
- Provider health snapshot covers workflow health, Google, banking, connectors, primary AI and optional Gemini fallback configuration.
- Groq remains the recommended primary OpenAI-compatible provider; optional Gemini fallback defaults to Google's OpenAI-compatible endpoint and is disabled for sensitive mail unless explicitly overridden.
- Android Today is exception-first, shows Autopilot health and `Needs you`, and background notifications are driven only by unavoidable human exceptions.
- Android version is `0.5.0+22`; backend version is `0.5.0`. Backend keeps `REQUIRED_ANDROID_VERSION = 0.4.16` for upgrade compatibility.

## Validation

The source passes Python `compileall` in the packaging environment. The packaging environment does not contain Flutter/Dart or `aiosqlite`, so the authoritative full gates remain the included GitHub Actions workflow: backend pytest, Flutter analyze/test, release APK build and signing. No claim is made that those external gates have run until GitHub Actions completes.
